...
using UnityEngine;
using PL = Assets._CrossyroadTest.Scripts.TwoBosses.Entities.Player.Player;

namespace Assets._CrossyroadTest.Scripts.View.TwoBosses.PlayerView
{
        ...
        protected override async UniTask MovePlayer(PB.MovePlayerCommand<PL> command, CancellationToken token)
        {
            await LSequence.Create()
                .Append(LMotion.Create(
                    ...
                    new Vector2(_levelStartPoint.x + _playerData.Position.x, _levelStartPoint.z + _playerData.Position.y), 0.3f).BindToPositionXZ(transform))
