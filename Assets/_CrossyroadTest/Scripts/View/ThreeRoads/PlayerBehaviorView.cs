...
namespace Assets._CrossyroadTest.Scripts.View.ThreeRoads
{
    ...
    public class PlayerBehaviorView : MonoBehaviour
    {
        ...
        private async UniTask PushPlayer(BehaviorCommand command, CancellationToken token)
        {
        ...
        private async UniTask DancePlayer(BehaviorCommand command, CancellationToken token)
        {
        ...
        private async UniTask MovePlayer(BehaviorCommand command, CancellationToken token)
        {
            await LSequence.Create()
                .Append(LMotion.Create(
                    new Vector2(transform.position.x, transform.position.z),
                    new Vector2(_worldObjectService.CurrentWorld.ZeroPosition.x + _playerData.Position.x, _worldObjectService.CurrentWorld.ZeroPosition.z + _playerData.Position.y),
