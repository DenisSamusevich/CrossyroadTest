...
namespace Assets._CrossyroadTest.Scripts.Common.Input
{
    internal class InputService
    {
        internal struct InputCommand<T>
        {
        ...
        private IPublisher<InputCommand<InputMovingType>> _inputMovingPublisher;
        private InputSystem_Actions _actions;
        private Camera _mainCamera;

        public InputService(Camera mainCamera, IPublisher<InputCommand<InputMovingType>> inputMovingPublisher)
        {
        ...
        public InputMovingType CurrentInput { get; private set; }
        ...
        private void OnTapPerformed(InputAction.CallbackContext context)
        {
            ...
            _inputMovingPublisher.Publish(new InputCommand<InputMovingType>(currentInput));
            ...
            InputMovingType CalculateCurrentInput(Vector3 playerPosition, Vector3 touchScreenPosition)
            {
                var touchPosition = touchScreenPosition - playerPosition;
                var currentInput = InputMovingType.None;
                if (Mathf.Abs(touchPosition.x) > Mathf.Abs(touchPosition.z))
                {
                    if (touchPosition.x > 0)
                    {
                        currentInput = InputMovingType.Right;
                    }
                    else
                    {
                        currentInput = InputMovingType.Left;
                    }
                }
                else
                {
                    ...
                    {
                        currentInput = InputMovingType.Forward;
                    }
                    else
                    {
                        currentInput = InputMovingType.Back;
                    }
                }
                return currentInput;
            }
